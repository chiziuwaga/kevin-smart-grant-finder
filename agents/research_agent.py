...
class ResearchAgent:
    ...
    async def _perform_deep_research(self, grant_title: str, grant_description: Optional[str]) -> Dict[str, Any]:
        """Performs deep research on a specific grant using Perplexity API."""
        if not self.perplexity_client:
            logger.error("Perplexity client not configured. Cannot perform deep research.")
            return {}

        # Construct a more detailed prompt for deep research
        # This prompt is a simplified example.
        llm_prompt = (
            f"Perform a detailed investigation of the grant titled '{grant_title}'. "
            f"Description (if available): '{grant_description or 'N/A'}'. "
            f"Focus on uncovering: "
            f"1. Specific eligibility requirements (e.g., organization type, project scope, geographic focus). "
            f"2. Key objectives and priorities of the funding agency for this grant. "
            f"3. Past funded projects or typical funding amounts, if available. "
            f"4. Application process overview and deadlines. "
            f"5. Any implicit criteria or preferences suggested by the grant documentation or agency website. "
            f"Please synthesize this information into a concise summary for each point. "
            f"If possible, return the response as a JSON object with keys like 'detailed_eligibility', 'funding_agency_priorities', 'past_funding_info', 'application_process', 'implicit_criteria'."
        )

        try:
            raw_fallback_response = await self.perplexity_client.chat.completions.create(
                model="llama-3-sonar-large-32k-online", # Or another suitable model for deep research
                messages=[
                    {"role": "system", "content": "You are a deep research analyst. Provide detailed insights into the specified grant based on available online information. Structure your findings in JSON if possible."},
                    {"role": "user", "content": llm_prompt}
                ]
            )
            # logger.debug(f"Raw Perplexity deep research response: {raw_fallback_response}")

            if raw_fallback_response and raw_fallback_response.choices:
                content_str = raw_fallback_response.choices[0].message.content
                try:
                    # Attempt to parse as JSON
                    research_details = json.loads(content_str)
                    return research_details
                except json.JSONDecodeError:
                    logger.warning(f"Deep research response was not JSON. Content: {content_str[:200]}...")
                    # Fallback: return the text content under a generic key
                    return {"detailed_summary_text": content_str}
            else:
                logger.warning("No response or empty choices from Perplexity for deep research.")
                return {}
        except Exception as e:
            logger.error(f"Error during deep research for grant '{grant_title}': {e}")
            return {}

    async def enrich_grant_details(self, grant: EnrichedGrant) -> EnrichedGrant:
        """Enriches a grant with more details using deep research and updates scores."""
        logger.info(f"Starting deep research for grant: {grant.title}")
        
        # Perform deep research
        additional_info = await self._perform_deep_research(grant.title, grant.description)
        grant.enrichment_log.append(f"Deep research performed at {datetime.now(timezone.utc).isoformat()}. Found: {list(additional_info.keys()) if additional_info else 'No new info'}")

        # Update grant fields based on additional_info
        # This is highly dependent on the structure of `additional_info`
        if additional_info:
            if not grant.summary_llm and additional_info.get("detailed_summary_text"):
                 grant.summary_llm = additional_info.get("detailed_summary_text")
            elif not grant.summary_llm and additional_info.get("funding_agency_priorities"): # Example
                 grant.summary_llm = str(additional_info.get("funding_agency_priorities"))

            if not grant.eligibility_criteria and additional_info.get("detailed_eligibility"):
                grant.eligibility_criteria = str(additional_info.get("detailed_eligibility"))
            
            # Example: Extracting keywords from deep research if not already present
            # This would ideally involve a proper NLP keyword extraction step
            if not grant.keywords and additional_info.get("detailed_summary_text"):
                # Simplistic keyword extraction (split and take unique words > 5 chars)
                words = set(additional_info.get("detailed_summary_text", "").lower().split())
                grant.keywords.extend([w for w in words if len(w) > 5 and w.isalpha()][:10]) # Limit to 10

            # Update raw_source_data or create a new field for deep research results
            if grant.raw_source_data:
                try:
                    existing_data = json.loads(grant.raw_source_data)
                    if isinstance(existing_data, dict):
                        existing_data["deep_research_info"] = additional_info
                        grant.raw_source_data = json.dumps(existing_data)
                    else: # If existing data is not a dict (e.g. a list or just a string)
                        grant.raw_source_data = json.dumps({"original_data": existing_data, "deep_research_info": additional_info})
                except json.JSONDecodeError:
                     # If original raw_source_data was not valid JSON, overwrite or append carefully
                     grant.raw_source_data = json.dumps({"original_raw": grant.raw_source_data, "deep_research_info": additional_info})
            else:
                grant.raw_source_data = json.dumps(additional_info)

        # Re-calculate scores with potentially new information
        # This assumes scoring methods can leverage the newly added details
        grant.research_scores.sector_relevance = await self._calculate_sector_relevance(grant)
        grant.research_scores.geographic_relevance = await self._calculate_geographic_relevance(grant)
        grant.research_scores.operational_alignment = await self._calculate_operational_alignment(grant)
        grant.research_scores.overall_score = (
            (grant.research_scores.sector_relevance or 0) +
            (grant.research_scores.geographic_relevance or 0) +
            (grant.research_scores.operational_alignment or 0)
        ) / 3
        grant.enrichment_log.append(f"Re-scoring after deep research at {datetime.now(timezone.utc).isoformat()}")

        logger.info(f"Deep research and re-scoring complete for grant: {grant.title}")
        return grant
