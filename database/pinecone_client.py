...
class PineconeClient:
    ...
    def _create_index(self):
        ...
        if self.use_mock:
            ...
            return
        # ... (keep existing real _create_index implementation)
        try:
    ...
    def calculate_relevance(self, grant_description, grant_title=None):
        ...
        try:
            ...
            # Query Pinecone
            # Note: This assumes you have priority vectors stored in Pinecone to compare against
            # If not, the relevance logic needs adjustment.
            results = self.index.query(
                vector=vector,
                top_k=5, # Query for top 5 matching priority vectors
                include_metadata=True
            )

            # Calculate weighted score based on matching priority vectors
            total_score = 0
            total_weight = 0

            if results.matches:
                for match in results.matches:
                    similarity = match.score # Cosine similarity (0 to 1)
                    # Assuming priorities have a 'weight' in metadata
                ...
                # Normalize score
                if total_weight > 0:
                    # Convert average cosine similarity to a 0-100 relevance score
                    normalized_score = max(0.0, min(100.0, (total_score / total_weight) * 100.0))
                else:
                    normalized_score = 0.0
            else:
                normalized_score = 0.0 # No matching priorities found

            return round(normalized_score, 2)

        except Exception as e:
            logging.error(f"Error calculating real relevance score: {str(e)}")
            return 0.0 # Default fallback score
    ...
    async def store_grant_embedding(self, grant_id: str, embedding: List[float], metadata: Dict[str, Any]) -> bool:
    ...
    # Add similar mocking logic to other methods if they are used before initialization is fixed
    # For example:
    async def find_similar_grants(self, query_embedding: List[float], top_k: int = 10, filter: Optional[Dict] = None) -> List[Dict]:
        if self.use_mock:
            logging.debug("Mock mode: Returning empty list for find_similar_grants")
            return []
        # --- Real implementation --- #
        # ... (keep existing real find_similar_grants implementation) ...
        try:
            if not self.index:
    ...
    async def update_grant_metadata(self, grant_id: str, metadata: Dict[str, Any]) -> bool:
        if self.use_mock:
             ...
             return True
        # --- Real implementation --- #
        # ... (keep existing real update_grant_metadata implementation) ...
        try:
            if not self.index:
    ...
    async def delete_grant(self, grant_id: str) -> bool:
        if self.use_mock:
            ...
            return True
        # --- Real implementation --- #
        # ... (keep existing real delete_grant implementation) ...
        try:
             if not self.index:
